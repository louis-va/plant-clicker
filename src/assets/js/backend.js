...
(() => {
  ...
  document.getElementById("boostPrice").innerHTML = getBoostPrice(
    currentUser.multiplier,
    currentUser.autoclick
  );
  ...
  document.getElementById("purchaseAutoClick").addEventListener("click", () => {
    if (currentUser.bank < getAutoClickerPrice()) {
      ...
      document.getElementById("amountAutoClick").innerHTML =
        currentUser.autoclick;
      document.getElementById("autoClickPrice").innerHTML =
        getAutoClickerPrice();
    }
  });

  document
    .getElementById("purchaseMultiplier")
    .addEventListener("click", () => {
      if (currentUser.bank < getMultiplierPrice()) {
        ...
        document.getElementById("amountMultiplier").innerHTML =
          currentUser.multiplier;
        document.getElementById("multiplierPrice").innerHTML =
          getMultiplierPrice();
      }
    });
...
function autoClick() {
  setScore(
    ...
    getMultiplier() * getAutoClicker()
  );
  setBank(
    JSON.parse(localStorage.getItem("user")),
    getMultiplier() * getAutoClicker()
  );
  document.getElementById("bank").innerHTML = JSON.parse(
